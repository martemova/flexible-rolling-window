...
for setup in lSetups:
    ...
    with tqdm(total=setup_total, unit="rep", desc=f"Setup {setup}", mininterval=60) as pbar:
        for wt in weight_type:
            ...
            for k, K in enumerate(lPar):
                ...
                for tau, iTau in enumerate(lRW):
                    pbar.set_postfix(wt=wt, K=K, RW=iTau)
                    ...
                    for rep in range(iD):
                        ...
                        if wt == 'exp':
                            ...
                            gamma_hat = float(1 - 0.5 * np.exp(res.x[0]) / (1 + np.exp(res.x[0]))) if res.success else 0.5
