...
def create_plots(G, RESULTS_DIR, E=1, support=None):
    """
    ...
    """
    plt.hist(G, bins=50, density=True, color='lightsteelblue', edgecolor='black')
    sns.kdeplot(data=G, color='red', lw=1.5)
    plt.title('Experiment ' + str(E), fontsize=18)
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.ylabel('')
    plt.savefig(RESULTS_DIR / f"E{E}.eps", format="eps")
    plt.show()
